...
int Main(int argc, char* argv[])
{
	...
	while ( projectElement != nullptr )
	{
		...
		if ( !global::NoIsoGen )
		{
			...
			dirTree->WriteDirectoryRecords( &writer, root, root, global::new_type.has_value() && *global::new_type ? dirTree->GetDirCountTotal() : 0 );
...
int ParseISOfileSystem(const tinyxml2::XMLElement* trackElement, const fs::path& xmlPath, iso::EntryList& entries, iso::IDENTIFIERS& isoIdentifiers, int& totalLen)
{
	...
	iso::DIRENTRY& root = iso::DirTreeClass::CreateRootDirectory(entries, volumeDate, ReadEntryAttributes(defaultAttributes, directoryTree).HFLAG);
	iso::DirTreeClass* dirTree = root.subdir.get();
