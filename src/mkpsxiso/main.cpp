...
int Main(int argc, char* argv[])
{
	...
	if(!global::RebuildXMLScript.empty())
	{
		...
		else
		{
			printf( "ERROR: Cannot open %s for writing\n", 
				global::RebuildXMLScript.string().c_str());
			return EXIT_FAILURE;
		}
		if ( !global::QuietMode )
		{
	...
	while ( projectElement != nullptr )
	{
		...
		if ( !global::QuietMode )
		{
			printf( "Building ISO Image: \"%s\"", global::ImageName.string().c_str() );

			if ( global::cuefile )
			{
				printf( " + \"%s\"", global::cuefile->string().c_str() );
			}
		...
		if ( !global::LBAfile.empty() )
		{
			...
			if (fp != nullptr)
			{
				...
				fprintf( fp, "Image bin file: \"%s\"\n", global::ImageName.string().c_str() );

				if ( global::cuefile )
				{
					fprintf( fp, "Image cue file: \"%s\"\n", global::cuefile->string().c_str() );
				}
			...
			if (fp != nullptr)
			{
				...
				if ( !global::QuietMode )
				{
					printf( "Wrote file LBA listing header \"%s\"\n\n",
						global::LBAheaderFile.string().c_str() );
				}
			}
			else
			{
